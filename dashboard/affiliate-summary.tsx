...
export function AffiliateSummary() {
  const [user] = useState({ affiliateCode: "DEMO123" }) // Mock user
  ...
  const referralLink = user?.affiliateCode
    ? `${typeof window !== "undefined" ? window.location.origin : ""}/auth?ref=${user.affiliateCode}`
    : ""
      ...
      <CardContent>
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold text-foreground">{totalConversions}</span>
          </div>
          <div className="flex justify-between items-center">
        ...
        <div className="bg-gradient-to-r from-purple-600/10 to-primary/10 rounded-lg p-4 mb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button size="icon" variant="outline" onClick={copyReferralLink} className="flex-shrink-0 bg-transparent">
              <Copy className="h-4 w-4" />
            </Button>
          </div>
        </div>

        <Button className="w-full nexarion-gradient text-white font-medium button-glow">Request Payout</Button>
      </CardContent>
    </Card>
  )
}
